...
contract RPS {
    ...
    function _checkWinnerAndPay() private {
        ...
        uint dist = p0Choice - p1Choice;
        //if (dist < 0 ) dist = dist*-1;
        if ((dist % 5)% 2 == p1Choice) { // all of the possibilities that acc1 wins
            // to pay player[1] for 2 ETH
            account1.transfer(reward);
        }
        else if ((dist % 5) % 2 == p0Choice) { // all of the possibilities that acc0 wins
